...
def submit_run(profile):
    ...
    run = w.jobs.submit_and_wait(
        run_name="databricks-opentelemetry-test",
        git_source=jobs.GitSource(
            git_url="https://github.com/christophergrant/databricks-opentelemetry.git",
        ...
        tasks=[
            jobs.SubmitTask(
                notebook_task=jobs.NotebookTask(
                    notebook_path="tests/test", source=jobs.Source.GIT
                ),
                new_cluster=compute.ClusterSpec(
                    cluster_log_conf=compute.ClusterLogConf(
...
with ThreadPoolExecutor(max_workers=len(profiles)) as executor:
    ...
    future_to_profile = {
        executor.submit(submit_run, profile): profile for profile in profiles
    }
